...
function formatBooking(ds: string) {
...
export default function CreateCaseScreen() {
  ...
  const needsBooking = tier === "video" || tier === "onsite" || tier === "workshop";
  ...
  const onSelectTier = () => {
    if (!tier) { Alert.alert("Bitte auswählen", "Wähle eine Inspektionsart aus."); return; }
    if (needsBooking && (!bookedDate || !bookedTime)) {
      Alert.alert("Termin fehlt", "Bitte wähle einen Termin für die Inspektion.");
      return;
    }
    goNext();
  };
  ...
  const onProcessPayment = async () => {
    if (!paymentMethod || !tier) { Alert.alert("Zahlungsart wählen", "Bitte wähle eine Zahlungsmethode."); return; }
    setPaymentProcessing(true);
    try {
      const result = await processPayment(tier, paymentMethod);
      if (result.success) { setTransactionId(result.transactionId); goNext(); }
      else Alert.alert("Zahlung fehlgeschlagen", result.error ?? "Bitte versuche es erneut.");
    } catch { Alert.alert("Fehler", "Zahlung konnte nicht verarbeitet werden."); }
    ...
    <Screen scroll>
        ...
        <View>
          ...
          <Controller control={control} name="vin" render={({ field: { onChange, value } }) => (
            <View style={s.vinRow}>
              ...
              <TouchableOpacity style={[s.scanBtn, { borderColor: colors.accent, backgroundColor: colors.accentLight }]} onPress={() => setShowVinScanner(true)} activeOpacity={0.7}>
          ...
          <Controller control={control} name="mileage" render={({ field: { onChange, value } }) => (
            <Input label="Kilometerstand in km (optional)" placeholder="z.B. 85000 oder leer lassen" keyboardType="number-pad" maxLength={6} value={value} onChangeText={onChange} error={errors.mileage?.message} />
          )} />
      ...
      {step === 1 && (
        <View>
          ...
          <TierPicker selected={tier} onSelect={(t) => { setTier(t); setBookedDate(null); setBookedTime(null); }} />

          {/* Booking card for video/onsite/workshop */}
          {needsBooking && (
            <>
              <Spacer size="md" />
              <TouchableOpacity
              ...
              >
                ...
                <View style={{ marginLeft: Spacing.md, flex: 1 }}>
                  {bookedDate && bookedTime ? (
                    <>
                      <Typo variant="body" style={{ fontWeight: "600" }}>{formatBooking(bookedDate)} um {bookedTime} Uhr</Typo>
                      <Typo variant="caption">Tippen zum Ändern</Typo>
                    </>
                  ) : (
                    <>
                      <Typo variant="body" style={{ fontWeight: "600" }}>Termin auswählen</Typo>
                      <Typo variant="caption">Datum und Uhrzeit für die Inspektion</Typo>
                    </>
                  )}
                </View>
      ...
      <BookingCalendar
        ...
        title={tier === "video" ? "Video-Termin" : tier === "onsite" ? "Vor-Ort-Termin" : "Werkstatt-Termin"}
        onConfirm={(date, time) => { setBookedDate(date); setBookedTime(time); setShowCalendar(false); }}
      ...
      {step === 2 && tier && (
        <View>
          ...
          <View style={[s.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
            <Typo variant="body" style={{ fontWeight: "700" }}>{SERVICE_TIER_LABELS[tier]}</Typo>
            <Spacer size="sm" />
            ...
            {bookedDate && bookedTime && (
              <View style={[s.featureRow, { marginTop: 4 }]}>
                ...
                <Typo variant="caption" style={{ marginLeft: 8 }}>{formatBooking(bookedDate)} um {bookedTime} Uhr</Typo>
              </View>
            )}
            <Spacer size="md" />
            <View style={[s.priceRow, { borderTopColor: colors.border }]}>
              <Typo variant="body" style={{ fontWeight: "600" }}>Gesamt</Typo>
              <Typo variant="h2" style={{ color: colors.accent }}>{TIER_PRICES[tier].toFixed(2).replace(".", ",")} €</Typo>
            </View>
          </View>
      ...
      {step === 3 && (
        <View>
          ...
          <Spacer size="md" />
          <Row label="FIN" value={getValues("vin").toUpperCase()} borderColor={colors.border} />
          <Row label="Marke" value={getValues("make")} borderColor={colors.border} />
          <Row label="Modell" value={getValues("variant") ? `${getValues("model")} ${getValues("variant")}` : getValues("model")} borderColor={colors.border} />
          <Row label="Baujahr" value={getValues("year")} borderColor={colors.border} />
          {getValues("mileage") ? <Row label="km-Stand" value={`${getValues("mileage")} km`} borderColor={colors.border} /> : null}
          {getValues("notes") ? <Row label="Notizen" value={getValues("notes")!} borderColor={colors.border} /> : null}
          <Row label="Art" value={tier ? SERVICE_TIER_LABELS[tier] : ""} borderColor={colors.border} />
          {bookedDate && bookedTime && <Row label="Termin" value={`${formatBooking(bookedDate)}, ${bookedTime} Uhr`} borderColor={colors.border} />}
          <Row label="Bezahlt" value={tier ? `${TIER_PRICES[tier].toFixed(2).replace(".", ",")} €` : ""} borderColor={colors.border} />
...
function Row({ label, value, borderColor }: { label: string; value: string; borderColor: string }) {
  return (
    <View style={[s.summaryRow, { borderBottomColor: borderColor }]}>
