      ...
      <View style={styles.skipRow}>
        {!isLast ? (
          <TouchableOpacity onPress={finishOnboarding} style={styles.skipBtn}>
            <Typo variant="caption" style={{ color: colors.textMuted }}>Überspringen</Typo>
          </TouchableOpacity>
        ) : <View />}
      </View>
      ...
      <View style={styles.bottomArea}>
        <View style={styles.dots}>
          {SLIDES.map((slide, i) => {
            ...
            return (
              <Animated.View key={i} style={[styles.dot, { width: dotWidth, opacity: dotOpacity, backgroundColor: slide.color }]} />
            );
          })}
        </View>
        <Spacer size="xl" />
        <View style={{ paddingHorizontal: Spacing.xl }}>
          <Button label={isLast ? "Loslegen" : "Weiter"} onPress={goNext} />
        </View>
        <Spacer size="xxl" />
      </View>
    </View>
...
const styles = StyleSheet.create({
  container: { flex: 1 },
  skipRow: { flexDirection: "row", justifyContent: "flex-end", paddingTop: 55, paddingHorizontal: Spacing.lg },
  skipBtn: { padding: Spacing.sm },
  slide: { flex: 1, alignItems: "center", justifyContent: "center", paddingBottom: 100 },
